...
def draw_fit_lines(img, lines, color=[255, 0, 0], thickness=5):
    ...
    # 왼쪽 대표선 그리기
    if left_lines:
        left_points = np.array(left_lines).reshape(-1, 4)
        left_fit_line = get_fitline(img, left_points)
        cv2.line(img, (left_fit_line[0], left_fit_line[1]), (left_fit_line[2], left_fit_line[3]), color, thickness)
    
    # 오른쪽 대표선 그리기
    if right_lines:
        right_points = np.array(right_lines).reshape(-1, 4)
        right_fit_line = get_fitline(img, right_points)
        cv2.line(img, (right_fit_line[0], right_fit_line[1]), (right_fit_line[2], right_fit_line[3]), color, thickness)
...
def hough_lines(img, rho, theta, threshold, min_line_len, max_line_gap):
    ...
    draw_fit_lines(line_img, lines)  # 수정된 함수 호출
    return line_img
...
def get_fitline(img, f_lines):
    try:
        ...
        print(f"get_fitline 함수에서 오류가 발생했습니다: {e}")
        return None
