...
# PID 제어기 초기화
pid_controller_left = PIDController(kp=4, ki=0.4, kd=0.05, log=True) # 초기 L : kp=4, ki=0.4, kd=0.05
pid_controller_right = PIDController(kp=4.75, ki=0.45, kd=0.02, log=True) # 초기 R : kp=4.75, ki=0.45, kd=0.02
...
while True:
    ...
    # PID 업데이트
    adjustment_left_turn = pid_controller_left.update(error_left_turn, dt)
    adjustment_right_turn = pid_controller_right.update(error_right_turn, dt)
    print("left val", adjustment_left_turn)
    print("right val", adjustment_right_turn)
    # 모터 속도 조절
    if predict == 0:  # 직진
        set_motor_speed('right', base_speed_r) #오른쪽
        set_motor_speed('left', base_speed_l) #왼쪽
        print("left_speed : ", base_speed_l, " rlght_speed : ",  base_speed_r)
    elif predict == 1:  # 좌회전
        set_motor_speed('right', base_speed_r + adjustment_left_turn)
        set_motor_speed('left', base_speed_l - adjustment_left_turn)
        print("left_speed : ",  base_speed_l - adjustment_left_turn, " rlght_speed : ", base_speed_r + adjustment_left_turn)
    elif predict == 2:  # 우회전
        set_motor_speed('right', base_speed_r - adjustment_right_turn)
        set_motor_speed('left', base_speed_l + adjustment_right_turn)
        print("left_speed : ", base_speed_l + adjustment_right_turn, " rlght_speed : ", base_speed_r - adjustment_right_turn)
